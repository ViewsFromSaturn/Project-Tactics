...
public partial class ChatPanel : Control
{
    ...
    private void BuildEmotePanel()
    {
        ...
        header.AddChild(UITheme.CreateDim("Ctrl+Enter to send", 9));
    ...
    private void BuildSettingsPanel()
    {
        ...
        _settingsPanel.CustomMinimumSize = new Vector2(260, 0);
        ...
        _settingsPanel.OffsetLeft = 0; _settingsPanel.OffsetRight = 260;
        ...
        _colorHexInput.FocusEntered += ResetIdle;
        colorRow.AddChild(_colorHexInput);

        _colorPreviewName = UITheme.CreateBody("Alaric", 12, _playerIcColor);
        ...
        _textSizeLabel.SizeFlagsHorizontal = SizeFlags.ExpandFill;
        sizeRow.AddChild(_textSizeLabel);
    ...
    {
        _chatFontSize = Math.Clamp(_chatFontSize + delta, 10, 18);
        _messageLog.AddThemeFontSizeOverride("normal_font_size", _chatFontSize);
        _chatInput.AddThemeFontSizeOverride("font_size", _chatFontSize);
    ...
    private void OnNameClicked(Variant meta)
    {
        ...
        GD.Print($"[Chat] Name clicked: {name} — profile popup not implemented yet");
    }
